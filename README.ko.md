...
**모니터는 동작보다 오래 남습니다.** 어떻게 열렸든 닫을 때까지 유지됩니다 — 그래프와 수치는 추출 중만이 아니라 끝난 뒤에도 볼 값이니까요. 정지 버튼은 멈출 것이 없으면 사라지고, 머리말은 화면에 있는 것을 계속 설명합니다. 끝난 스팀은 페이지를 닫거나 다음 동작이 시작될 때까지 그 설정과 함께 *스팀*으로 남습니다.
...
개수 제한은 없습니다. 캐러셀은 한 번에 몇 개씩만 그리고, 아래 스트립은 가로로 스크롤하며 가운데 카드를 항상 보이게 합니다.
...
그라인더는 **브랜드**와 **모델**, 그리고 버에 관한 것들입니다.

**✨ AI로 채우기** 는 두 이름을 받아 버 세트·형상·지름과 사용감 몇 줄을 채웁니다. 버 크기는 **절대 추측하지 않도록** 지시되어 있습니다. 모르는 그라인더는 그럴듯한 값 대신 빈 채로 돌아옵니다 — 틀린 64mm 하나가 이후의 모든 판단을 오염시키기 때문입니다. 채워진 값은 전부 수정할 수 있습니다.

게이트웨이는 그라인더를 이름 하나로 저장하므로 브랜드와 모델은 합쳐서 넘기고, 나뉘는 지점만 스킨이 기억합니다.

**무엇으로 갈았는지가 나머지에 전달됩니다.** 샷의 AI 분석과 원두의 프로파일 추천 모두 그라인더를 함께 받습니다 — 형상·크기·버 세트·특성, 그리고 그때의 분쇄도까지. 그래서 분쇄도 조정이 **그 그라인더의 다이얼** 기준으로 제안되고, 프로파일은 그 버가 감당할 수 있는지로 저울질됩니다.
...
| | |
|---|---|
| 원두 · 그라인더 | 그 샷이 기록한 로스터·원두·그라인더·분쇄도. 다른 원두가 물려 있었거나 분쇄도를 나중에 적은 경우 여기서 고칩니다 |
...
- **WebSocket 다섯 개 중 넷을 닫고** 깨어날 때 다시 엽니다

마지막 항목은 프로세서가 아니라 무선에 대한 것입니다. 물 수위는 잠금화면에 나오지 않는데 초당 한 패킷을 쓰고, 저울·기기·디스플레이 채널은 아무것도 보내지 않으면서 연결만 붙들고 있습니다. 머신 스냅샷만 남깁니다 — 그룹헤드에서 깨운 머신을 알아채는 통로가 그것뿐이기 때문입니다.

머신을 실제로 idle ↔ sleeping으로 구동하며 측정한 값:

| | 깨어있음 | 절전 |
|---|---|---|
| CPU | 26 ms/s | **0.6 ms/s** |
| WebSocket 프레임 | 2.8 /s | **1.8 /s** |

저울은 이 스킨이 관리할 대상이 아닙니다. 게이트웨이의 *저울 전원 모드*가 머신이 잠들면 이미 연결을 끊습니다.
...
| 키 | |
|---|---|
설정은 **게이트웨이**의 키-값 네임스페이스 `liquid-aqua`에 저장되고, 브라우저에는 사본이 남습니다.

게이트웨이는 스킨 generation마다 다른 포트로 서빙하므로, 업데이트할 때마다 브라우저는 새 origin에 놓이고 `localStorage`는 빈 상태가 됩니다. 거기에만 둔 값은 다음 패치에서 사라집니다. 저장소는 generation을 넘어 남고, 로컬 사본은 시작을 빠르게 하며 게이트웨이에 닿지 않을 때도 동작하게 합니다.

| 게이트웨이에 저장 | |
|---|---|
| `prefs` | 언어, 단위, 나이트 모드, 모니터·절전 옵션, 물 보정, Gemini 키 |
| `favs` | 즐겨찾기 프로파일 |
| `presets` | 스팀·플러시·온수 프리셋 |
| `shotai` | AI 추출 분석 결과, 대상 샷 기준 |

업데이트 후 첫 실행에서 저장소가 비어 있으면 이 브라우저에 남아 있는 값을 올려보내므로, 기존 설정이 사라지지 않고 이관됩니다.
